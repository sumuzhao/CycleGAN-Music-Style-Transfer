...
class cyclegan(object):
    def __init__(self, sess, args):
        ...
        self.now_datatime = get_now_datetime()
    ...
    def _build_model(self):
        ...
        self.d_sum = tf.summary.merge([self.da_loss_sum, self.da_loss_real_sum, self.da_loss_fake_sum, self.db_loss_sum,
                                       ...
                                       self.d_all_loss_sum, self.D_loss_sum])
    ...
    def train(self, args):
        ...
        log_dir = './logs/{}2{}_{}_full_1'.format(self.dataset_A_dir, self.dataset_B_dir, self.now_datatime)
        # log_dir = './logs/{}2{}_{}_extra-D-JCP'.format(self.dataset_A_dir, self.dataset_B_dir, '2018-06-10')
        self.writer = tf.summary.FileWriter(log_dir, self.sess.graph)

        # sample_z = np.random.normal(0, 1, size=(self.batch_size, self.z_dim))
        # dataA = np.load('./datasets/jazz/tra_phr_jazz_train.npy')
        # dataB = np.load('./datasets/pop/tra_phr_pop_train.npy')
        ...
        for epoch in range(args.epoch):
            # Load training data from 2 domains A and B
            np.random.shuffle(dataA)
            np.random.shuffle(dataB)
            np.random.shuffle(data_mixed)
            # Get the proper number of batches
            batch_idxs = min(min(len(dataA), len(dataB)), args.train_size) // self.batch_size
            # batch_idxs = min(dataA.shape[0], dataB.shape[0], args.train_size) // self.batch_size
            ...
            for idx in range(0, batch_idxs):
                ...
                gaussian_noise = np.abs(np.random.normal(0, 1, [self.batch_size, self.time_step, self.pitch_range,
                                                                self.input_c_dim]))
                ...
                # batch_files = dataB[idx * self.batch_size:(idx + 1) * self.batch_size]
                # batch_midis = [pretty_midi_to_piano_roll(batch_file) for batch_file in batch_files]
                # batch_midis = np.array(batch_midis).astype(np.float32)
                # batch_z = np.random.normal(0, 1, [self.batch_size, self.z_dim]).astype(np.float32)
                # batch_A = dataA[idx * self.batch_size:(idx + 1) * self.batch_size] * 2. - 1.
                # batch_B = dataB[idx * self.batch_size:(idx + 1) * self.batch_size] * 2. - 1.
                # batch_images = np.concatenate((batch_A, batch_B), axis=3)
                ...
                fake_A, fake_B, _, summary_str, g_loss_a2b, g_loss_b2a, cycle_loss, g_loss = self.sess.run([self.fake_A,
                    ...
                    self.g_loss], feed_dict={self.real_data: batch_images, self.gaussian_noise: gaussian_noise,
                                             self.lr: lr, self.real_mixed: batch_images_mixed})
                ...
                if np.mod(counter, args.print_freq) == 1:
                    sample_dir = os.path.join(self.sample_dir, '{}2{}_{}_full_1'.format(self.dataset_A_dir, self.dataset_B_dir,
                                                                                 self.now_datatime))
                    # sample_dir = os.path.join(self.sample_dir, '{}2{}_{}_extra-D-JCP_0.08'.format(self.dataset_A_dir, self.dataset_B_dir,
                    #                                                           '2018-06-10'))
    ...
    def save(self, checkpoint_dir, step):
        ...
        # model_dir = "{}2{}_{}_full_1".format(self.dataset_A_dir, self.dataset_B_dir, self.now_datatime)
        model_dir = "{}2{}_{}_extra-D-JCP_1".format(self.dataset_A_dir, self.dataset_B_dir, '2018-06-14')
    ...
    def sample_model(self, sample_dir, epoch, idx):
        ...
        # Load testing data from 2 domains A and B
        dataA = glob('./datasets/{}/train/*.*'.format(self.dataset_A_dir))
        dataB = glob('./datasets/{}/train/*.*'.format(self.dataset_B_dir))
        ...
        # batch_files = dataB[:self.batch_size]
        # sample_midis = [pretty_midi_to_piano_roll(batch_file) for batch_file in batch_files]
        # sample_midis = np.array(sample_midis).astype(np.float32)
        batch_files = list(zip(dataA[:self.batch_size], dataB[:self.batch_size]))
        sample_images = [load_npy_data(batch_file) for batch_file in batch_files]
        sample_images = np.array(sample_images).astype(np.float32)
        # sample_images = np.concatenate((dataA[:self.batch_size] * 2. - 1., dataB[:self.batch_size] * 2. - 1.), axis=3)
    ...
    def test(self, args):
        ...
        test_dir_mid = os.path.join(args.test_dir, '{}2{}_{}_full_1/{}/mid'.format(self.dataset_A_dir, self.dataset_B_dir,
                                                                            self.now_datatime, args.which_direction))
        if not os.path.exists(test_dir_mid):
            os.makedirs(test_dir_mid)

        test_dir_npy = os.path.join(args.test_dir, '{}2{}_{}_full_1/{}/npy'.format(self.dataset_A_dir, self.dataset_B_dir,
                                                                            self.now_datatime, args.which_direction))
        ...
        for idx in range(len(sample_files)):
            ...
            # sample_npy_re = sample_npy.reshape(1, sample_npy.shape[0], sample_npy.shape[1], sample_npy.shape[2])
