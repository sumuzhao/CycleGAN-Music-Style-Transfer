...
class Classifer(object):
    ...
    def train(self, args):
        ...
        log_dir = './logs/classifier_{}2{}_{}_{}'.format(self.dataset_A_dir, self.dataset_B_dir, self.now_datatime,
                                                         str(self.sigma))
        ...
        gaussian_noise = np.random.normal(0, self.sigma, [data_test.shape[0], data_test.shape[1],
        ...
        for epoch in range(args.epoch):
            ...
            for idx in range(batch_idx):
                ...
                # gaussian_noise = np.random.normal(0, self.sigma, [batch_data.shape[0], batch_data.shape[1],
                #                                                   batch_data.shape[2], batch_data.shape[3]])
                # batch_data = batch_data.reshape(batch_data.shape[0], batch_data.shape[1], batch_data.shape[2], 1)
                # batch_data = batch_data + gaussian_noise
    ...
    def save(self, checkpoint_dir, step):
        ...
        # model_dir = "classifier_{}2{}_{}_{}".format(self.dataset_A_dir, self.dataset_B_dir, self.now_datatime,
        #                                             str(self.sigma))
    ...
    def test(self, args):
        ...
        sample_files_origin = glob('./test/{}2{}_{}_extra-D-JCP/{}/npy/origin/*.*'.format(self.dataset_A_dir,
                                                                              self.dataset_B_dir,
                                                                              '2018-06-13',
                                                                              args.which_direction))
        sample_files_origin.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split('_')[0]))
        sample_files_transfer = glob('./test/{}2{}_{}_extra-D-JCP/{}/npy/transfer/*.*'.format(self.dataset_A_dir,
                                                                                  self.dataset_B_dir,
                                                                                              '2018-06-13',
                                                                                  args.which_direction))
        sample_files_transfer.sort(key=lambda x: int(os.path.splitext(os.path.basename(x))[0].split('_')[0]))
        sample_files_cycle = glob('./test/{}2{}_{}_extra-D-JCP/{}/npy/cycle/*.*'.format(self.dataset_A_dir,
                                                                            self.dataset_B_dir,
                                                                                        '2018-06-13',
                                                                            args.which_direction))
        ...
        test_dir_mid = os.path.join(args.test_dir, '{}2{}_{}_extra-D-JCP/{}/mid_attach_prob'.format(self.dataset_A_dir,
                                                                                        self.dataset_B_dir,
                                                                                                    '2018-06-13',
                                                                                        args.which_direction))
        ...
        for idx in range(len(sample_files)):
            ...
            # gaussian_noise = np.random.normal(0, self.sigma, [sample_origin.shape[0], sample_origin.shape[1],
            #                                                   sample_origin.shape[2], sample_origin.shape[3]])
