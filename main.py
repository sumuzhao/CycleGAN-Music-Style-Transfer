...
parser.add_argument('--dataset_A_dir', dest='dataset_A_dir', default='JAZZ', help='path of the dataset of domain A')
parser.add_argument('--dataset_B_dir', dest='dataset_B_dir', default='POP', help='path of the dataset of domain B')
...
parser.add_argument('--sigma', dest='sigma', type=float, default=1.0, help='sigma of gaussian noise')
...
def main(_):
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        # model.train(args) if args.phase == 'train' else model.test_famous(args)

        classifier = Classifer(sess, args)
        classifier.train(args) if args.phase == 'train' else classifier.test_famous(args)

        # musegan_model = cyclegan(sess, args)
        # musegan_model.train(args) if args.phase == 'train' else musegan_model.gen_test()
